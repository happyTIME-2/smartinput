<template>
	<span
		...
		:style="styleObject">
			<slot></slot>
			<i class="base-tag__close base-icon-close" v-if="closeable" @click='close'>&#xe69e;</i>
	</span> 
</template>
<script>
export default {
	...
	created() {
		this.className = this.effect ? `base-tag base-tag--${this.effect}` : 'base-tag';
		this.className = this.type ? `${this.className} base-tag--${this.type}` : this.className;
		this.className = this.size ? `${this.className} base-tag--${this.size}` : this.className;
